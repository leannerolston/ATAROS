#Open files:
gridPath$ = "/Users/rolston/Desktop/ATAROS/fine-textgrids/"
...
for g from 1 to gridLength
    ...
    #transcription speaker1
	intervals_ch1 = Get number of intervals: 3 
    ...
    #coarse speaker1 
	intervals_ch2 = Get number of intervals: 4 
	...
	for i from 1 to intervals_ch1
		...
		#get fine annotations- get endpoints of selection, calculate 
        #midpoint, then check value on tier 5
        start = Get starting point... 3 i
        end = Get end point... 3 i
        mid = start + (end-start)/2
        #appendInfoLine: end, tab$, start, tab$, mid
		fine_interval = Get interval at time... 5 mid
        fine$ = Get label of interval... 5 fine_interval
        appendInfoLine: "fine: " + fine$

		appendFileLine: file_name$ + "_fine.txt", speech$, tab$, stance$, tab$, task$, tab$, fine$
	endfor

		
	#Get info from speaker 2: 
    #speaker 2 transcription 
	intervals_ch3 = Get number of intervals: 8 
	interval_name$ = Get tier name: 8

	#check that number of intervals matches:
	intervals_ch4 = Get number of intervals: 9
	...
	for i from 1 to intervals_ch3
		speech$ = Get label of interval: 8, i
		stance$ = Get label of interval: 9, i
		appendInfoLine: "speech: " + speech$
		appendInfoLine: "stance: " + stance$ 
		appendInfoLine: "task: " + task$
		...
		#get fine annotations- get endpoints of selection, calculate 
        #midpoint, then check value on tier 5
        start = Get starting point... 8 i
        end = Get end point... 8 i
        mid = start + (end-start)/2
        #appendInfoLine: end, tab$, start, tab$, mid
		fine_interval = Get interval at time... 10 mid
        fine$ = Get label of interval... 10 fine_interval
        appendInfoLine: "fine:" + fine$
